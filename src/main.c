...
int main( void )
{
    ...
    // CVE-2022-39274
    printf("Using LoRaMac-Node Version: 4.6.0\n"); // GITHUB_VERSION
    printf("Using STM32L4 CMSIS Version: 1.5.0\n"); // __STM32L4_CMSIS_VERSION
    printf("Using STM32L4 HAL Version: 1.9.0\n"); // STM32L4XX_HAL_VERSION
    printf("Using " MBEDTLS_VERSION_STRING_FULL "\n");
    volatile uint32_t v3 = HAL_GetHalVersion();
    (void) v3;
    volatile unsigned int v4 = mbedtls_version_get_number(); 
    (void) v4;

    (void) HAL_RNG_Init(&hrng);
    // some dummy code so mbedTLS doesn't get thrown out
    ...
    mbedtls_ssl_handshake(&ctx);

    // Explicity trigger CVE-2024-23775
    ...
    printf("Calling into mbedtls_x509_set_extension() now.. fingers crossed!\n");
    (void) mbedtls_x509_set_extension(&(csr_ctx.extensions),
